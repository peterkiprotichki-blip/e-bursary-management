"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PropertyTenantRepository = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const base_repository_1 = require("../../database/repositories/base.repository");
const property_tenant_schema_1 = require("../schemas/property-tenant.schema");
let PropertyTenantRepository = class PropertyTenantRepository extends base_repository_1.BaseRepository {
    constructor(model) {
        super(model);
    }
    async findBySystemTenant(tenantId) {
        return this.model.find({ tenantId, isDeleted: false }).sort({ createdAt: -1 }).exec();
    }
    async findByProperty(tenantId, propertyId) {
        return this.model.find({ tenantId, currentPropertyId: propertyId, isDeleted: false }).exec();
    }
    async findByEmail(tenantId, email) {
        return this.model.findOne({ tenantId, email, isDeleted: false }).exec();
    }
    async countBySystemTenant(tenantId) {
        return this.model.countDocuments({ tenantId, isDeleted: false });
    }
    async countActive(tenantId) {
        return this.model.countDocuments({ tenantId, isActive: true, isDeleted: false });
    }
    async countByProperty(tenantId, propertyId) {
        return this.model.countDocuments({ tenantId, currentPropertyId: propertyId, isDeleted: false });
    }
    async countActiveByProperty(tenantId, propertyId) {
        return this.model.countDocuments({ tenantId, currentPropertyId: propertyId, isActive: true, isDeleted: false });
    }
};
exports.PropertyTenantRepository = PropertyTenantRepository;
exports.PropertyTenantRepository = PropertyTenantRepository = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(property_tenant_schema_1.PropertyTenant.name)),
    __metadata("design:paramtypes", [mongoose_2.Model])
], PropertyTenantRepository);
//# sourceMappingURL=property-tenant.repository.js.map